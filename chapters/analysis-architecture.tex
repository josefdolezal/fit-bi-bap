...
Z alternativních architektur jsem nakonec zvolil Model-View-ViweModel, kterou porovnám s dopručeným \acrshort{mvc}.
...
\begin{figure}\centering
	...
	\caption[Architektura \acrshort{mvc}]{Architektura \acrshort{mvc}}\label{fig:architektura-mvc}
\end{figure}
...
\swiftcode{code:mvc-view-controller}{ViewControlle architektuře \acrshort{mvc}}{assets/code/mvc-view-controller.swift}
...
Z tohoto důvodu se \acrshort{mvc} často přezdívá \uv{Massive View Controller} \ref{fig:massive-mvc}.
...
\begin{figure}\centering
	...
	\caption[Role Controlleru v \acrshort{mvc}]{Controller spjatý s View}\label{fig:massive-mvc}
\end{figure}

\subsection{\acrshort{mvvm}: Model-View-ViewModel}\label{analyza-mvvm}
...
\begin{description}
  ...
  Oproti \acrshort{mvc} tedy perzistentní data nejsou viditelná Controlleru, ale pouze View Modelu.
...
Pro porovnání architektury s \acrshort{mvc} lze opět využít scénář pro stažení dat. Pro tento scénář by se architektura \acrshort{mvvm} chovala následovně:
\begin{itemize}
...
Toto lze vyřešit pomocí \acrshort{kvo}, které nabízí Apple v základu.
\acrshort{kvo} umožňuje objektu zaregistrovat se na notifikace o změně stavu nějakého libovolného objektu.
...
Z tohoto důvodu jsem jako architekturu vybral \acrshort{mvvm} s použitím reaktivních rozšíření místo standardního \acrshort{mvc}.
