...
Tato architektura rozděluje aplikaci do tří vrstev: Model, View a \texttt{Controller}.
...
\begin{description}
  ...
  Jedním z úkolů \texttt{Controlleru} je striktně zamezit přímé interakci mezi View a Modelem.
  ...
  Při rozdělení do obrazovek platí pravidlo, že jeden \texttt{Controller} obsluhuje jedno nebo více View.
...
Z tohoto shrnutí vyplývá, že \texttt{Controller} je velmi blízce spjat s View. Toto propojení reprezentuje obrázek \ref{fig:massive-mvc}.
...
\begin{itemize}
  ...
  \item Tuto interakci odchytí View a upozorní \texttt{Controller}.
  ...
  \item \texttt{Controller} aktualizuje View.
  \item Nastane-li během stahování chyba, \texttt{Controller} vytváří nové View a chybu prezentuje uživateli.
\end{itemize}
...
Kvůli přímému napojení \texttt{Controlleru} na View se při testování jeho chování (behavior testing) musí využít simulátoru mobilního operačního systému.
...
V ideálním případě je \texttt{Controller} zodpovědný pouze za správné sestavení View a napojení zfromátovaných výstupů na něj.
...
\begin{itemize}
  \item \texttt{Controller} napojuje výstupy ViewModelu na View a vytváří pravidla pro převod uživatelské interakce na vstupy ViewModelu.
  \item Uživatel v aplikaci klikne na tlačítko \uv{Stáhnout data}.
  \item View upozorňuje \texttt{Controller} na interakci uživatele, ten automaticky vytváří vstup pro ViewModel.
  \item ViewModel na základě vstupu stahuje data a předává je Modelu.
  \item Model po uložení notifikuje ViewModel, ten vytváří výstup pro \texttt{Controller}, který nechává překreslit View.
  \item V případě chyby vytvoří ViewModel chybový výstup, ten se pomocí \texttt{Controlleru} propaguje do View.
\end{itemize}
...
\begin{itemize}
  ...
  \item View vykresluje na obrazovku naformátované výstupy a upozorňuje \texttt{Controller} při interakci uživatele,
  \item \texttt{Controller} sestavuje hierarchii View, napojuje zformátované výstupy ViewModelu na View a z uživatelské interakce vytváří vstupy pro ViewModel,
...
ViewModel generuje výstupy na základě vstupů, v testech tedy lze uživatelskou interakci podvrhnout a testovat pouze výstupy (není potřeba vytvářet View ani \texttt{Controller}).
